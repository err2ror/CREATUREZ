# CHONKER v3.3 – CREATUREZ+ (centralised synergy + nerd stats)
import random
import os
import shutil
import datetime
...
def compute_team_bonus(team, debug=False):
    """
    CENTRAL BATTLE API — all team bonus logic lives here.
    Returns:
      - if debug=False: final_multiplier
      - if debug=True: (final_multiplier, nerd_info_dict)
    """
    info = {}

    if not team:
        if debug:
            return 1.0, {"base_bonus": 1.0, "color_bonus": 1.0,
                         "rarity_bonus": 1.0, "avg_rarity": 0.0,
                         "final_bonus": 1.0}
        return 1.0

    # base bonus (old behavior)
    base = 0.9 + 0.1 * len(team)
    info["base_bonus"] = base

    # color synergy
    colors = {c["color"] for c in team}
    color_bonus = 1.05 if len(colors) == 1 else 1.0
    info["color_bonus"] = color_bonus
    ...
    info["rarity_bonus"] = rarity_bonus
    info["avg_rarity"] = avg_rarity

    final = base * color_bonus * rarity_bonus
    info["final_bonus"] = final
...
def calctot(team, out="fp", chaos=0.2):
    ...
    bonus, nerd = compute_team_bonus(team, debug=True)
    val = val * bonus
    savval = val
    ...
    if out == "log":
        nerd_info = {
            "chaos": chaosf,
            "bonus": bonus,
            "no_chaos_value": savval,
            "initval": initval,
            "base_bonus": nerd["base_bonus"],
            "color_bonus": nerd["color_bonus"],
            "rarity_bonus": nerd["rarity_bonus"],
            "avg_rarity": nerd["avg_rarity"],
            "final_bonus": nerd["final_bonus"]
        }
        return {"0": val, "nerd": nerd_info}
    else:
        return val
...
def battle(a, b, randomdebug: list):
    ...
    print("=== BATTLE START ===")
    print(f"Team A (Power {rdba['initval']} * {round(rdba['chaos'], 3)} = {rdba['no_chaos_value']})")
    print(f"  base bonus: {rdba['base_bonus']}")
    print(f"  color bonus: {rdba['color_bonus']}")
    print(f"  rarity bonus: {rdba['rarity_bonus']} (avg rarity {rdba['avg_rarity']})")
    print(f"  final team bonus: {rdba['final_bonus']}")
    print("VS")
    print(f"Team B (Power {rdbb['initval']} * {round(rdbb['chaos'], 3)} = {rdbb['no_chaos_value']})")
    print(f"  base bonus: {rdbb['base_bonus']}")
    print(f"  color bonus: {rdbb['color_bonus']}")
    print(f"  rarity bonus: {rdbb['rarity_bonus']} (avg rarity {rdbb['avg_rarity']})")
    print(f"  final team bonus: {rdbb['final_bonus']}")
    print("====================")
...
def crepowahthing():
    ...
    while True:
        ...
        bonus, nerd = compute_team_bonus(team, debug=True)
        base_power = x * num
        totpowah = base_power * bonus

        print(f"\n=== {num} COPIES OF {xn} ===")
        print(f"Base power: {base_power}")
        print(f"Base bonus: {nerd['base_bonus']}")
        print(f"Color bonus: {nerd['color_bonus']}")
        print(f"Rarity bonus: {nerd['rarity_bonus']} (avg rarity {nerd['avg_rarity']})")
        print(f"FINAL TEAM BONUS: {nerd['final_bonus']}")
        print(f"TOTAL POWER: {totpowah}")
        print(f"VS {yn}: {y} power (diff {y - totpowah})")
...
def clean_monsterdb():
    ...
    def clean(i):
        ...
        return {
            ...
            "power_level": power,
            "rarity": i.get("rarity", get_rarity(power)),
            "traits": i.get("traits", []),
