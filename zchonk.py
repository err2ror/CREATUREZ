...
RARITY_INDEX = {r: i for i, r in enumerate(RARITY_LIST)}
...
def calctot(team, out="fp", chaos=0.2):
    ...
    if out == "log":
        return {
            "0": val,
            "nerd": [chaosf, bonus, savval, initval]  # chaos, bonus, no chaos value, init
        }
    else:
        return val
...
def clean_monsterdb():
    ...
    def clean(i):
        ...
        return {
            ...
            "rarity": i.get("rarity", random.choice(RARITY_LIST)),
            "traits": i.get("traits", []),
